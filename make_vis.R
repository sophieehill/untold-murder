...
people <- people %>% mutate(image = paste0("https://raw.githubusercontent.com/sophieehill/untold-murder/main/photos/compressed/", gsub(" ", "_", id), ".png"),
                            label = id,
                            size = case_when(id=="Daniel Morgan" ~ 100,
                                             id=="Jonathan Rees" ~ 80,
                                             id=="Sid Fillery" ~ 80,
                                             id=="Southern Investigations" ~ 80,
                                             TRUE ~ 60))
...
visNetwork(people, connections, width = "100%", height = "500px", 
           ...
           submain=list(text="A visualization based on reporting in the podcast/book, <a href='http://www.untoldmurder.com/' style='color:#fff9ef'>Untold Murder</a>",
                        style='font-family:"Big Brother", sans-serif;font-size:30px;text-align:center;color:#fff9ef;')) %>%
  ...
  visNodes(
    ...
    shape = case_when(people$type!="person" ~ "image",
                      TRUE ~ "circularImage"),
  ...
  visInteraction(hover=TRUE, zoomView = TRUE,
                ...
                -moz-border-radius: 3px;-webkit-border-radius: 3px;border-radius: 3px;
                 ...
                 max-width:280px;overflow-wrap: normal;margin-left: 70px;') %>%
